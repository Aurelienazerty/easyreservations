...
class ExportXLS {
	...
	public function addHeader($header) {
		...
		if(is_array($header))$this->headerArray[] = $header;
		else $this->headerArray[][0] = $header;
	}
	...
	public function addRow($row) {
		...
		if(is_array($row)) {
			...
			} else $this->bodyArray[] = $row;
		} else $this->bodyArray[][0] = $row;
	}
	...
	*/
	...
	private function buildXLS() {
		...
		#build headers
		if(is_array($this->headerArray)) $xls .= $this->build($this->headerArray);
		#build body
		if(is_array($this->bodyArray)) $xls .= $this->build($this->bodyArray);
	...
	#build and return the headers 
		...
		foreach($array as $row){
			...
			foreach($row as $field) {
				if(is_numeric($field)) $build .= $this->numFormat($this->rowNo, $colNo, $field);
				else $build .= $this->textFormat($this->rowNo, $colNo, $field);
				$colNo++;
			}
			$this->rowNo++;
		}
	...
	# format and return the field as a header
		if(isset($_POST['charset']) && $_POST['charset'] != 'UTF-8') $data = iconv("utf-8", $_POST['charset'], $data);
		else $data = mb_convert_encoding($data, 'ISO-8859-1');
		$length = strlen($data);
